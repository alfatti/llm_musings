...
def build_prompt(new_email: str, examples: List[LabeledEmail]) -> str:
    ...
    # Strong guardrails + clear instructions
    prompt = f"""
...
You must classify the email into exactly ONE category chosen strictly from the ALLOWED CATEGORIES list below.
You are NOT permitted to create new categories, rewrite categories, combine categories, or output any wording
that is not verbatim from the allowed list.

If none of the allowed categories applies, you MUST return:
"category": "Other"

Any category not exactly matching one of the allowed category strings is invalid.
...
- Be concise but specific enough for the Middle Office to act without re-reading the original email.
...
def classify_email(
    ...
    - Build the prompt
    - Call Gemini via OpenAI client with JSON schema guardrails
    ...
    # 3) Call the model via Responses API with JSON schema + enum constraint
    ...
    # 4) Parse JSON and basic post-processing
